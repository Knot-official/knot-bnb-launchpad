...
/// @notice Fixed supply, no transfer tax or upgrade path. Optional immutable market taxes fund holder rewards.
contract LaunchToken is ERC20Burnable, ReentrancyGuard {
    ...
    IPoolManager public rewardManager;
        ...
        address locker, address treasury, address[] calldata assets) external
    {
        ...
        require(hook.code.length > 0 && manager.code.length > 0 && treasury != address(0)
        ...
        rewardManager = IPoolManager(manager);
    ...
    /// @notice Called after the curve transfers tax or the hook mints its funded settlement claims.
    function accrueTax(address asset, uint256 gross, uint256 amount, bool isBuy) external nonReentrant {
        require(taxEnabled && (msg.sender == taxCurve || msg.sender == taxHook) && isRewardAsset[asset], "TAX_SOURCE");
        require(amount > 0 && amount == marketTax(gross, isBuy), "TAX_AMOUNT");
        _accountFunding(asset, amount);
        uint256 protocol = Math.min(amount / 10, Math.mulDiv(gross, 30, 10000));
        uint256 remainder = amount - protocol;
        uint256 dividends = Math.mulDiv(remainder, taxConfig.extraDividendBps, 10000);
        uint256 buyback = Math.mulDiv(remainder, taxConfig.extraBuybackBps, 10000);
        uint256 creator = remainder - dividends - buyback;
        _fundDividends(asset, dividends);
        pendingBuyback[asset] += buyback;
        taxCredits[asset][taxTreasury] += protocol;
        address escrow = IRewardFactory(factory).vault();
        taxCredits[asset][IRewardFeeVault(escrow).creators(address(this))] += creator;
        emit TaxAccrued(asset, isBuy, gross, amount, protocol, creator, dividends, buyback);
    }
    ...
    function fundDividends(address asset, uint256 amount) external nonReentrant {
        require(msg.sender == IRewardFactory(factory).vault() && isRewardAsset[asset], "FEE_VAULT");
        _accountFunding(asset, amount);
    ...
    /// @dev Called outside swaps so PoolManager claims can be redeemed without a nested unlock.
    ...
    /// @dev The external self-call isolates failed ERC20 transfers and V4 redemptions per recipient.
    ...
    function _payReward(address asset, address recipient, uint256 amount) private {
        ...
        if (held < amount) rewardManager.unlock(abi.encode(asset, amount - held));
    ...
    function rewardBacking(address asset) public view returns (uint256) {
        ...
        return IERC20(asset).balanceOf(address(this)) + rewardManager.balanceOf(address(this), uint256(uint160(asset)));
    }

    /// @dev Pool taxes are funded ERC6909 claims; redeem only during this token's guarded claim.
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        require(msg.sender == address(rewardManager) && _reentrancyGuardEntered(), "REWARD_CALLBACK");
        (address asset, uint256 amount) = abi.decode(data, (address, uint256));
        require(isRewardAsset[asset], "REWARD_ASSET");
        rewardManager.burn(address(this), uint256(uint160(asset)), amount);
        uint256 beforeBalance = IERC20(asset).balanceOf(address(this));
        rewardManager.take(Currency.wrap(asset), address(this), amount);
        require(IERC20(asset).balanceOf(address(this)) - beforeBalance == amount, "OUTPUT_TAX");
        return "";
    }
